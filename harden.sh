...
#!/bin/bash
# Weekly Auditd summary report (all available events)

REPORT=$(/usr/sbin/aureport --summary -i 2>&1)

if [[ $? -eq 0 ]]; then
    echo -e "Weekly Auditd report.\n\n$REPORT" | mail -s "WEEKLY AUDITD REPORT - $(hostname)" root
    exit 0
else
    echo -e "Aureport error.\n\n$REPORT" | mail -s "AUDITD ERROR - $(hostname)" root
    exit 1
fi
EOF_AUDITD
